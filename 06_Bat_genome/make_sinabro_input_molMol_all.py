...
molMol_bed = Path(PurePath(bat_genome_path, "HLmolMol2.Bat1Kannotation.bed"))
molMol_fasta = Path(PurePath(bat_genome_path, "HLmolMol2.fa"))

# output files
# output for Sinabro
output_file_cdsplus_name = f"HLmolMol2.all.pc_transcripts.cdsplus.fa"
output_file_cdsplus = Path(PurePath(bat_genome_path, output_file_cdsplus_name))

# output for sanity check
nopass_names = []
for i in range(1,8,1):
    nopass_names.append(f"HLmolMol2.all.pc_transcripts.parity{i}.fa")

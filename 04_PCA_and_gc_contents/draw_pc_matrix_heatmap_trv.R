...
trv_trs <- read.csv(trv_trs_file)

trv_trs_noseqerr <- trv_trs %>% arrange(SBS_transition) %>% filter(Signature %in% signatures_noseqerr)

pcs <- c()
for (i in 1:length(pc_matrix$Principal_component)){pcs <- c(pcs, paste("PC", as.character(i), sep=""))}

pcm_heatmap <- ggplot(pc_matrix, aes(x = factor(Principal_component, levels=pcs), y = factor(Signature, levels = trv_trs_noseqerr$Signature),fill=Coefficient)) +
  geom_tile() +
  scale_fill_gradient2(midpoint=0, low="blue", mid="white",
                       high="red") +
  xlab("Principal Component") +
  ylab("Signature Sorted by transition ratio") +
...
out_file <- paste(base_path, "pc_matrix_noseqerr_trs.png", sep="")
